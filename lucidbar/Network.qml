...
Item {
    ...
    // dropped on narrow screens so the pill degrades to icon-only instead of
    // overflowing the bar's horizontal budget
    ...
    readonly property bool shown: Prefs.showNetwork
    ...
    readonly property int openHeight: expanded ? Math.min(root.maxPanelHeight, Math.max(120, listCol.implicitHeight + 190)) : root.compactHeight
    ...
    readonly property int popupHeight: Math.min(root.maxPanelHeight, Math.max(120, listCol.implicitHeight + 190))
    ...
    Component.onCompleted: root.refreshActiveNetwork()

    // safety net in case connectionFailed never fires and we're stuck "Connecting..."
    Timer {
        ...
        Item {
            ...
            Row {
                ...
                Item {
                    ...
                    EthernetGlyph {
                        anchors.fill: parent
                        visible: root.primaryIsEthernet
                    ...
                    WifiStrengthGlyph {
                        ...
                        showOff: !root.wifiConnected
                        color: root.wifiConnected ? Theme.accent : Theme.subtext
                    }
                ...
                Text {
                    // tied to showLabel too: this baselines off statusLabel,
                    // which the Row skips once that label goes invisible
                    visible: root.showLabel && (root.wifiConnected || root.ethernetConnected)
